 ...
 * paiement. S'appuie sur les fixtures seedées par CommissionsSeeder
 * (Commission vente / propriétaire) et CommissionLogistiqueSeeder (logistique).
 */
...
test('détail Commission vente — 5 cartes et tabs identiques', async ({
    ...
    const row = page
        .locator('tbody tr', { hasText: /Mamadou\s+SOUMAH/i })
        .first();
    await expect(row).toBeVisible({ timeout: 20_000 });
    await row.click();
    ...
    if (await periodeSelect.isVisible({ timeout: 5_000 }).catch(() => false)) {
        ...
        if (await option.isVisible({ timeout: 3_000 }).catch(() => false)) {
            await option.click();
            await expect(page).toHaveURL(/\/comptabilite\/commissions\/vente\/livreurs\//, {
                timeout: 15_000,
            });
            await assertSummaryCardsAndTabs(page, 'Frais');
        }
    ...
    // Le solde restant (1 500 GNF, non versé) doit afficher le bouton Payer.
...
test('détail Commission propriétaire — libellé « Frais véhicules »', async ({
    ...
    const row = page
        .locator('tbody tr', { hasText: /Fatoumata\s+DIALLO/i })
        .first();
    await expect(row).toBeVisible({ timeout: 20_000 });
    await row.click();
    ...
    await page.getByRole('button', { name: 'Dépenses', exact: false }).click();
    await expect(page.locator('body')).toContainText(/véhicules|aucune dépense/i);
});

...
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .org-slug  { font-size: 8px; color: #6b7280; margin-top: 2px; text-transform: uppercase; letter-spacing: 1px; }
...
<div class="page">
    ...
    <div class="header">
        <div>
            <div class="org-name">{{ $org?->nom ?? 'ELM' }}</div>
            <div class="org-slug">{{ $org?->slug ?? '' }}</div>
        </div>
        <div class="doc-title">
            <div class="doc-type">Liste des dépenses</div>
            <div class="doc-date">Édité le {{ $generated_at->format('d/m/Y à H:i') }}</div>
        </div>
    </div>
    ...
    <div class="meta">
        ...
        @endif
        @if($site_nom)
            <div>Site : <span>{{ $site_nom }}</span></div>
        @endif
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $row['site']['nom'] ?? '—' }}</td>
                <td>{{ $row['user']['name'] ?? '—' }}</td>
                <td>{{ $row['validateur_nom'] ?? '' }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="9" style="text-align:center; color:#6b7280; padding: 20px;">Aucune dépense pour ces critères.</td>
            </tr>
            @endforelse

            @if($rows->count() > 0)
            <tr class="total-row">
                <td colspan="4" style="text-align:right; padding-right:8px; color:#374151;">TOTAL</td>
    ...
    <div class="footer">
        <span>{{ $org?->nom ?? 'ELM' }} — Document confidentiel</span>

...
const props = defineProps<{
    ...
    kpis: { nb_livreurs: number; total_brut: number; total_frais?: number; total_net: number; total_verse: number; solde_total: number };
    search: string;
...
function exportExcel() {
    window.open('/comptabilite/commissions/vente/export/excel?' + buildParams().toString(), '_blank');
}

function exportPdf() {
    window.open('/comptabilite/commissions/vente/export/pdf?' + buildParams().toString(), '_blank');
}

const kpiTotalFrais = computed(() => props.kpis.total_frais ?? props.beneficiaires.reduce((s, b) => s + b.total_frais, 0));
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 p-6">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight">Commission livreur vente</h1>
                    <p class="mt-1 text-sm text-muted-foreground">
                        {{ kpis.nb_livreurs }} livreur{{ kpis.nb_livreurs !== 1 ? 's' : '' }}
                    </p>
                </div>
            ...
            <div class="grid grid-cols-3 gap-3 sm:grid-cols-5">
                <div class="rounded-lg border bg-card p-4 text-center">
                    <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Brut cumulé</p>
                ...
                <table v-if="beneficiaires.length > 0" class="w-full text-sm">
                    <thead>
                        <tr class="border-b bg-muted/40">
                            ...
                            <th class="px-5 py-3.5 text-left font-medium text-muted-foreground">Véhicule</th>
                    ...
                    <tbody class="divide-y">
                        <tr
                            ...
                            @click="router.visit('/comptabilite/commissions/vente/livreurs/' + b.beneficiaire_id)"
                        >
                            <td class="px-5 py-4">

...
import { Calculator, CheckCircle, Download, Lock, Trash2 } from 'lucide-vue-next';
...
const props = defineProps<{
    ...
    can: { calculer: boolean; valider: boolean; cloturer: boolean; delete: boolean };
}>();
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: props.periode.reference, href: `/comptabilite/periodes/${props.periode.id}` },
];
...
function doCalculer() {
    router.post(`/comptabilite/periodes/${props.periode.id}/calculer`, {}, {
        onSuccess: () => toast.add({ severity: 'success', summary: 'Fiches générées', life: 3000 }),
    });
}

function doValider() {
    confirm.require({
        message: 'Valider cette période ? Les fiches ne pourront plus être recalculées.',
        header: 'Confirmer la validation',
        acceptLabel: 'Valider',
        rejectLabel: 'Annuler',
        accept: () => router.post(`/comptabilite/periodes/${props.periode.id}/valider`),
    });
}
...
function doCloturer() {
    confirm.require({
        ...
        rejectLabel: 'Annuler',
        accept: () => router.post(`/comptabilite/periodes/${props.periode.id}/cloturer`),
    });
}
...
function exportExcel() {
    window.open(`/comptabilite/fiches/export/excel?periode_id=${props.periode.id}`, '_blank');
}
</script>
        ...
        <div class="flex flex-col gap-6 p-6">
            ...
            <div class="flex items-start justify-between">
                <div>
                    <div class="flex items-center gap-3">
                        <h1 class="text-xl font-semibold font-mono">{{ periode.reference }}</h1>
                        <span
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">
                        {{ periode.type_label }} —
                        {{ periode.date_debut }} au {{ periode.date_fin }}
                        <span v-if="periode.site"> — {{ periode.site.nom }}</span>
                    </p>
                    <p v-if="periode.observations" class="mt-1 text-xs text-muted-foreground italic">
                ...
                <div class="flex items-center gap-2">
                    ...
                    >
                        ...
                        {{ periode.statut === 'brouillon' ? 'Générer les fiches' : 'Recalculer' }}
                    </Button>
                    <Button
                        v-if="can.valider"
                        size="sm"
            ...
            <div class="grid gap-3 sm:grid-cols-4">
                <div class="rounded-xl border bg-card p-4">
                    ...
                    <p class="mt-1 text-lg font-bold">{{ fmt(stats.total_brut) }}</p>
                </div>
                <div class="rounded-xl border bg-card p-4">
                    <p class="text-xs text-muted-foreground">Total déductions</p>
                    <p class="mt-1 text-lg font-bold text-red-600 dark:text-red-400">-{{ fmt(stats.total_deductions) }}</p>
                </div>
                <div class="rounded-xl border bg-card p-4">
                    <p class="text-xs text-muted-foreground">Net à payer</p>
                    <p class="mt-1 text-lg font-bold text-emerald-600 dark:text-emerald-400">{{ fmt(stats.total_net) }}</p>
                </div>
                <div class="rounded-xl border bg-card p-4">
                    <p class="text-xs text-muted-foreground">Payé / Reste</p>
                    <p class="mt-1 text-lg font-bold">{{ fmt(stats.total_paye) }}</p>
                    <p class="text-xs text-muted-foreground">{{ stats.nb_paye }} payé · {{ stats.nb_a_payer }} à payer</p>
                </div>
            </div>
            ...
            <div class="overflow-hidden rounded-xl border bg-card">
                <div class="border-b px-5 py-3">
                    <h2 class="text-sm font-semibold">{{ fiches.length }} fiche{{ fiches.length !== 1 ? 's' : '' }}</h2>
                </div>
                <DataTable :value="fiches" data-key="id" striped-rows class="text-sm">
                    <Column header="Bénéficiaire" style="min-width: 200px">
                        <template #body="{ data }">
                            ...
                            <div class="text-xs text-muted-foreground font-mono">{{ data.reference }}</div>
                        </template>
                    </Column>
                    ...
                    <template #empty>
                        <div class="py-16 text-center text-sm text-muted-foreground">
                            Aucune fiche générée. Cliquez sur "Générer les fiches" pour lancer le calcul.
                        </div>
                    </template>
                </DataTable>
            </div>

...
const roleConfig: Record<string, { label: string; color: string; bg: string }> = {
    super_admin:      { label: 'Super Admin',      color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-50 dark:bg-violet-950/40' },
    admin_entreprise: { label: 'Admin Entreprise', color: 'text-blue-600 dark:text-blue-400',     bg: 'bg-blue-50 dark:bg-blue-950/40' },
    commerciale:      { label: 'Commerciale',      color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/40' },
    comptable:        { label: 'Comptable',        color: 'text-amber-600 dark:text-amber-400',   bg: 'bg-amber-50 dark:bg-amber-950/40' },
};
...
const cfg = computed(() => roleConfig[props.role.name] ?? { label: props.role.name, color: 'text-muted-foreground', bg: 'bg-muted' });

const activePermissions = ref<Set<string>>(
    new Set(
        isSuperAdmin.value
            ? props.resources.flatMap(r => props.actions.map(a => `${r}.${a}`))
...
const totalPossible = computed(() => props.resources.length * props.actions.length);
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div
                        ...
                        :class="cfg.bg"
                    >
                        <ShieldCheck v-if="role.name === 'super_admin'" class="h-5 w-5" :class="cfg.color" />
                        <Shield v-else-if="role.name === 'admin_entreprise'" class="h-5 w-5" :class="cfg.color" />
                        <Users v-else-if="role.name === 'commerciale'" class="h-5 w-5" :class="cfg.color" />
                        <Lock v-else class="h-5 w-5" :class="cfg.color" />
                    </div>

                    <div>
                        <div class="flex items-center gap-2">
                            <h1 class="text-xl font-semibold">{{ cfg.label }}</h1>
                            <Badge v-if="isSuperAdmin" variant="secondary" class="text-[10px] text-violet-600 dark:text-violet-400">
                                Protégé · Bypass automatique
                            </Badge>
                        </div>
                        <p class="text-sm text-muted-foreground">
                            {{ role.users_count }} {{ role.users_count <= 1 ? 'utilisateur' : 'utilisateurs' }} ·
                            <span class="font-medium">{{ totalChecked }}</span> / {{ totalPossible }} permissions actives
                        </p>
                    </div>
                </div>
            ...
            <div class="overflow-hidden rounded-xl border bg-card shadow-sm">
                                ...
                                >
                                    <div class="flex flex-col items-center gap-2">
                                        <span
                                            ...
                                            :class="actionLabels[action]?.color ?? 'text-muted-foreground'"
                                        >
                                        ...
                                        >
                                            <!-- État: tout coché -->
                                            <CheckSquare
                                                v-if="columnState(action) === 'all'"
                                                class="h-4 w-4"
                                            />
                                            <!-- État: partiel -->
                                            <Minus
                                                v-else-if="columnState(action) === 'partial'"
                            ...
                            >
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        >
                                            <Minus
                                                v-if="rowState(resource) !== 'none'"
                                                class="h-3.5 w-3.5"
                                                :class="rowState(resource) === 'all' ? 'text-primary-foreground' : 'text-primary'"
                                            />
                                        </button>
                                ...
                                >
                                    <div class="flex justify-center">
                                        ...
                                        >
                                            <Check v-if="isChecked(resource, action)" class="h-3 w-3" />
                                        </button>
                                    </div>
                                </td>
                            </tr>
                ...
                <div class="flex items-center justify-between border-t bg-muted/20 px-6 py-3">
                    <div class="flex items-center gap-4 text-xs text-muted-foreground">
                        ...
                        <span class="flex items-center gap-1.5">
                            <span class="inline-block h-2 w-2 rounded-sm border-2 border-primary/60 bg-primary/10" />
                            Partiel
                        </span>
                    </div>
                    ...
                    <p v-else class="text-xs text-muted-foreground italic">
                        Les permissions du Super Admin sont gérées automatiquement.
                    </p>
                </div>
            </div>
        </div>

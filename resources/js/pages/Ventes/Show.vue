...
const progressPercent = computed(() => {
    ...
    return Math.min(
        100,
        Math.round((props.facture.montant_encaisse / props.facture.montant_net) * 100),
    );
});
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :hide-mobile-header="true">
        ...
        <div class="mr-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
            ...
            <div class="rounded-xl border bg-card p-4 shadow-sm sm:p-5">
                <h3 class="mb-5 text-sm font-semibold tracking-wider text-muted-foreground uppercase">
                    Informations
                </h3>
                ...
                <!-- Dates de transition -->
                <div v-if="commande.validated_at" class="mt-4 flex flex-wrap gap-4 border-t pt-4 text-xs text-muted-foreground">
                    <span>Validée le <strong>{{ commande.validated_at }}</strong></span>
                    <span v-if="commande.created_by">par <strong>{{ commande.created_by }}</strong></span>
                </div>
                ...
                >
                    <p class="mb-1 text-xs font-medium tracking-wider text-red-600 uppercase dark:text-red-400">
                        Motif d'annulation
                    </p>
            ...
            <div v-if="facture" class="rounded-xl border bg-card p-4 shadow-sm sm:p-5">
                ...
                <div class="mb-6 grid gap-4 sm:grid-cols-3">
                    ...
                    <div class="rounded-lg bg-emerald-50 p-4 dark:bg-emerald-950/30">
                        ...
                        <p class="mt-0.5 text-lg font-bold text-emerald-600 tabular-nums dark:text-emerald-400">
                            {{ formatGNF(facture.montant_encaisse) }}
                        </p>
                    </div>
                    <div
                        class="rounded-lg p-4"
                        :class="facture.montant_restant > 0 ? 'bg-amber-50 dark:bg-amber-950/30' : 'bg-emerald-50 dark:bg-emerald-950/30'"
                    >
                        <p
                            class="text-xs"
                            :class="facture.montant_restant > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'"
                        >
                            Restant dû
                        </p>
                        <p
                ...
                <div v-if="canAddEncaissement" class="rounded-lg border bg-muted/20 p-4">
                    ...
                    <form class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" @submit.prevent="submitEncaissement">
                        <!-- Montant -->
                        <div>
            ...
            >
                La facture sera créée automatiquement lors de la validation de la commande.
            </div>
        </div>

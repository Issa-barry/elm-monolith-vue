...
interface Vehicule { id: string; nom_vehicule: string; immatriculation: string; }
interface Site { id: string; nom: string; type: string; }
...
const form = useForm({
    ...
    vehicule_id:     props.depense.vehicule_id ?? '',
                ...
                <form class="space-y-5" @submit.prevent="submit">
                    ...
                    <div>
                        <Label for="dep-type" class="mb-1.5 block text-xs font-medium">
                            Type de dépense <span class="text-destructive">*</span>
                        </Label>
                        <select
                            id="dep-type"
                    ...
                    <div>
                        <Label for="dep-vehicule" class="mb-1.5 block text-xs font-medium">
                            Véhicule
                            <span v-if="selectedType?.requires_vehicle" class="text-destructive">*</span>
                        </Label>
                        <select
                            id="dep-vehicule"
                            v-model="form.vehicule_id"
                            class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm"
                            :class="{ 'border-destructive': form.errors.vehicule_id }"
                        >
                    ...
                    <div>
                        <Label for="dep-comment" class="mb-1.5 block text-xs font-medium">
                            Commentaire
                            <span v-if="selectedType?.requires_comment" class="text-destructive">*</span>
                        </Label>
                        <textarea
                            id="dep-comment"
                            v-model="form.commentaire"
                            rows="3"
                            class="flex min-h-[72px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                    ...
                    <!-- Statut -->
                    <div>
                        <Label class="mb-2 block text-xs font-medium">Statut</Label>
                        <select
                            v-model="form.statut"

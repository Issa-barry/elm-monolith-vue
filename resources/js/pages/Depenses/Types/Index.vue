...
import DepenseTypeImportDialog from './partials/DepenseTypeImportDialog.vue';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Types de dépense', href: '/backoffice/depenses/types' },
];
...
// ── Export (respecte les filtres actifs — jamais seulement la page affichée) ──
...
function exportExcel() {
    window.open(
        `/backoffice/depenses/types/export/excel?${buildExportParams()}`,
        '_blank',
    );
}
...
function exportPdf() {
    window.open(
        `/backoffice/depenses/types/export/pdf?${buildExportParams()}`,
        '_blank',
    );
}
...
function handleSubmit() {
    if (editingType.value) {
        form.put(`/backoffice/depenses/types/${editingType.value.id}`, {
            preserveScroll: true,
            onSuccess: () => {
        ...
        form.post('/backoffice/depenses/types', {
            preserveScroll: true,
            onSuccess: () => {
                showDialog.value = false;
                form.reset();
...
function toggle(type: DepenseType) {
    router.patch(
        `/backoffice/depenses/types/${type.id}/toggle`,
        {},
        { preserveScroll: true },
    );
}
...
function destroy(type: DepenseType) {
    ...
    router.delete(`/backoffice/depenses/types/${type.id}`, {
        preserveScroll: true,
    });
}
        ...
        <div class="flex flex-col gap-6 p-4 sm:p-6">
            ...
            <div class="overflow-hidden rounded-xl border bg-card">
                ...
                >
                    ...
                    <!-- Concerné (catégorie, pas un statut — fond coloré autorisé) -->
                    <Column
                    ...
                    >
                        <template #body="{ data }">
                            <StatusDot
                                :status="data.is_active ? 'actif' : 'inactif'"

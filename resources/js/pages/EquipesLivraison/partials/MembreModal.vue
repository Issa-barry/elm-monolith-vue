...
const title = computed(() => (isEdit.value ? 'Modifier le membre' : 'Nouveau membre'));
...
watch(
    ...
    (val) => {
        if (!val) return;
        (Object.keys(errors) as (keyof MembreFormData)[]).forEach((k) => delete errors[k]);
        ...
        <div class="space-y-4 pb-1 pt-2">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    />
                    <p v-if="maxTauxSafe < 100" class="mt-1 text-xs text-muted-foreground">
                        Maximum disponible : {{ maxTauxSafe }}%
                    </p>
                    <p v-if="errors.taux_commission" class="mt-1 text-xs text-destructive">

...
const progressPercent = computed(() => {
    ...
    return Math.min(
        100,
        Math.round((props.packing.montant_verse / props.packing.montant) * 100),
    );
});
...
const canAddVersement = computed(
    () =>
        props.packing.statut !== 'annulee' &&
        props.packing.montant_restant > 0 &&
        can('packings.update'),
);
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :hide-mobile-header="true">
        ...
        >
            ...
            <p
                class="flex-1 truncate text-center font-mono text-sm font-semibold"
            >
                {{ packing.reference }}
            </p>
            <Link
        ...
        <div class="mx-auto max-w-4xl space-y-6 p-4 sm:p-6">
            ...
            <div class="hidden items-start justify-between gap-4 sm:flex">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <DropdownMenu
                        v-if="packing.can_cancel && can('packings.update')"
                    >
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <p class="mt-0.5 font-medium tabular-nums">
                            {{ packing.nb_rouleaux.toLocaleString('fr-FR') }}
                        </p>
                    </div>
                    <div>
                        <p class="text-xs text-muted-foreground">
                        ...
                        <p class="mt-0.5 font-medium tabular-nums">
                            {{ formatGNF(packing.prix_par_rouleau) }}
                        </p>
                    </div>
            ...
            >
                ...
                <p class="text-sm whitespace-pre-wrap">{{ packing.notes }}</p>
            </div>
            ...
            <div class="rounded-xl border bg-card p-6 shadow-sm">
                ...
                <div class="mb-6 space-y-2">
                    <div
                        class="flex flex-wrap items-center justify-between gap-1 text-sm"
                    >
                        <span class="text-muted-foreground">
                            Versé :
                            <span
                                class="font-semibold text-foreground tabular-nums"
                                >{{ formatGNF(packing.montant_verse) }}</span
                            >
                            / {{ formatGNF(packing.montant) }}
                        </span>
                        <span
                    ...
                    >
                        ...
                        <div>
                            ...
                            <Calendar
                                ...
                                input-class="w-full"
                                :class="{
                                    'p-invalid': versementForm.errors.date,
                                }"
                            />
                            <p
                                v-if="versementForm.errors.date"
                                class="mt-1 text-xs text-destructive"
                            >
                                {{ versementForm.errors.date }}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <InputNumber
                                ...
                                input-class="w-full"
                                :class="{
                                    'p-invalid': versementForm.errors.montant,
                                }"
                            />
                            <p

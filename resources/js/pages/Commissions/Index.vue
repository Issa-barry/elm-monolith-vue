        ...
        list = list.filter(
            (p) =>
                (p.commande_reference &&
                    p.commande_reference.toLowerCase().includes(query)) ||
                (p.vehicule_nom &&
                    p.vehicule_nom.toLowerCase().includes(query)) ||
                (p.immatriculation &&
                    p.immatriculation.toLowerCase().includes(query)) ||
...
const partsFiltrees = computed(() =>
    filterList([...props.parts], search.value),
);
const mobileFiltrees = computed(() =>
    filterList([...props.parts], mobileSearch.value),
);
...
const kpi = computed(() => {
    ...
    return {
        ...
        total_a_verser: list
            ...
            .reduce((s, p) => s + p.montant_restant, 0),
        ...
        <div class="flex flex-col sm:hidden">
            ...
            <div class="grid grid-cols-2 gap-3 p-4">
                <div class="rounded-xl border bg-card p-3 shadow-sm">
                    ...
                    <p
                        class="mt-1 text-base font-bold text-amber-600 tabular-nums dark:text-amber-400"
                    >
                        {{ formatGNF(kpi.total_a_verser) }}
                    </p>
                </div>
                <div class="rounded-xl border bg-card p-3 shadow-sm">
                    ...
                    <p
                        class="mt-1 text-base font-bold text-amber-600 tabular-nums dark:text-amber-400"
                    >
                        {{ formatGNF(kpi.montant_en_attente) }}
                    </p>
                    <p class="text-xs text-muted-foreground">
                        {{ kpi.nb_en_attente }} part{{
                            kpi.nb_en_attente > 1 ? 's' : ''
                        }}
                    </p>
                </div>
                <div class="rounded-xl border bg-card p-3 shadow-sm">
                    ...
                    <p
                        class="mt-1 text-base font-bold text-emerald-600 tabular-nums dark:text-emerald-400"
                    >
            ...
            <div class="divide-y">
                ...
                >
                    <div class="min-w-0 flex-1">
                        ...
                        <p
                            v-if="p.equipe_nom && tab === 'livreurs'"
                        ...
                        <p class="mt-0.5 text-sm font-semibold tabular-nums">
                            {{ formatGNF(p.montant_net) }}
                        </p>
                        <p
                            v-if="p.montant_restant > 0"
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="rounded-xl border bg-card p-5 shadow-sm">
                    ...
                    <p class="mt-2 text-2xl font-bold tabular-nums">
                        {{ formatGNF(kpi.total_brut) }}
                    </p>
                    <p class="mt-0.5 text-xs text-muted-foreground">
                        {{ kpi.nb_parts }} part{{ kpi.nb_parts > 1 ? 's' : '' }}
                    </p>
                </div>
                        ...
                        <div class="flex items-center gap-3">
                            <IconField class="max-w-sm flex-1">
                                ...
                                <InputText
                                    ...
                                    :placeholder="
                                        tab === 'livreurs'
                                            ? 'Commande, livreur, équipe, véhicule…'
                                            : 'Commande, propriétaire, véhicule…'
                                    "
                            ...
                            <span class="text-xs text-muted-foreground">
                                {{ partsFiltrees.length }} résultat{{
                                    partsFiltrees.length !== 1 ? 's' : ''
                                }}
                            </span>
                        </div>
                    </template>
                    ...
                    >
                        <template #body="{ data }">
                            ...
                            <p
                                v-if="
                                    tab === 'livreurs' &&
                                    data.livreur_principal_telephone
                                "
                    ...
                    >
                        <template #body="{ data }">
                            ...
                            <p
                                v-if="
                                    tab === 'proprietaires' &&
                                    data.frais_supplementaires > 0
                                "
                                class="mt-0.5 text-xs text-destructive tabular-nums"
                            >
                                −
                                {{ formatGNF(data.frais_supplementaires) }}
                                frais
                            </p>
                        </template>
                    </Column>
                    ...
                    <Column header="" style="width: 110px">
                        <template #body="{ data }">
                            <div class="flex justify-end">
                                <Link
                                    :href="`/commissions/${data.commission_id}`"
                                >
                                    <Button
                                        size="sm"
                                        variant="ghost"

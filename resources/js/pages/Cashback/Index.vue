...
const kpi = computed(() => {
    ...
    const enCours = all.filter(
        (t) => t.statut === 'valide' || t.statut === 'partiel',
    );
    return {
...
function formatGNF(v: number) {
    return new Intl.NumberFormat('fr-FR').format(v) + ' GNF';
}
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :hide-mobile-header="true">
        ...
        <div class="flex flex-col sm:hidden">
            ...
            <div
                v-if="flashSuccess"
                class="mx-4 mt-3 flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-xs text-green-800"
            >
            ...
            <div class="grid grid-cols-2 gap-3 p-4">
                ...
                <div class="rounded-xl border bg-card p-4 shadow-sm">
                    ...
                    <p class="mt-1 text-lg font-bold tabular-nums">
                        {{ formatGNF(kpi.montant_attente) }}
                    </p>
                    <p class="text-xs text-muted-foreground">
                        {{ kpi.nb_attente }} transaction{{
                            kpi.nb_attente !== 1 ? 's' : ''
                        }}
                    </p>
                </div>
                <div class="rounded-xl border bg-card p-4 shadow-sm">
                    ...
                    <p class="mt-1 text-lg font-bold tabular-nums">
                        {{ formatGNF(kpi.montant_cours) }}
                    </p>
                    <p class="text-xs text-muted-foreground">
                        {{ kpi.nb_cours }} transaction{{
                            kpi.nb_cours !== 1 ? 's' : ''
                        }}
                    </p>
                </div>
                <div class="rounded-xl border bg-card p-4 shadow-sm">
                    ...
                    <p class="mt-1 text-lg font-bold tabular-nums">
                        {{ formatGNF(kpi.montant_verse) }}
                    </p>
                    <p class="text-xs text-muted-foreground">
                        {{ kpi.nb_verse }} transaction{{
                            kpi.nb_verse !== 1 ? 's' : ''
                        }}
                    </p>
                </div>
            ...
            <div class="divide-y">
                ...
                >
                    <div class="min-w-0 flex-1">
                        <p class="font-medium">
                            {{ t.client?.nom_complet ?? '—' }}
                        </p>
                        <p
                            v-if="t.client?.telephone"
                            class="mt-0.5 text-xs text-muted-foreground"
                        >
                            {{ formatPhone(t.client.telephone) }}
                        </p>
                        <p class="mt-1 text-sm font-semibold tabular-nums">
                            {{ formatGNF(t.montant) }}
                        </p>
                        <p
                            v-if="t.montant_restant > 0"
                            class="text-xs font-semibold text-orange-600 tabular-nums"
                        >
                    ...
                    <div class="flex shrink-0 flex-col items-end gap-2">
                        ...
                        <Button
                            v-else-if="
                                t.statut === 'valide' || t.statut === 'partiel'
                            "
                            size="sm"
                            variant="outline"
                ...
                >
                    ...
                    >
                        <template #body="{ data }">
                            ...
                            <div
                                v-if="data.client?.telephone"
                                class="text-xs text-muted-foreground"
                            >
                                {{ formatPhone(data.client.telephone) }}
                            </div>
                        </template>
                    </Column>
                    ...
                    >
                        <template #body="{ data }">
                            <StatusDot
                                :label="
                                    statutConfig[data.statut]?.label ??
                                    data.statut
                                "
        ...
        <Dialog
            ...
            :header="
                historyTarget
                    ? `Historique — ${historyTarget.client?.nom_complet ?? ''}`
                    : 'Historique'
            "
            :style="{ width: '560px' }"
        >
            <div v-if="historyTarget">
                <div
                    v-if="historyTarget.versements.length === 0"
                ...
                <table v-else class="w-full text-sm">
                    ...
                    <tbody>
                        <tr
                            v-for="v in historyTarget.versements"
                            :key="v.id"
        ...
        >
            <div v-if="validerTarget" class="flex flex-col gap-4">
                <div class="rounded-lg border bg-muted/30 px-4 py-3 text-sm">
                    ...
                    <p class="font-medium">
                        {{ validerTarget.client?.nom_complet }}
                    </p>
                ...
                >
                    En validant, vous autorisez le versement (total ou partiel)
                    de ce cashback.
                </p>
                <div>
                    ...
                    <textarea
                        ...
                        rows="2"
                        class="w-full rounded-lg border bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
            ...
            <template #footer>
                <div class="flex justify-end gap-2">
                    <Button variant="outline" @click="validerVisible = false"
                        >Annuler</Button
                    >
                    <Button
        ...
        >
            <div v-if="verserTarget" class="flex flex-col gap-4">
                ...
                <div class="rounded-lg border bg-muted/30 px-4 py-3 text-sm">
                    <p class="text-muted-foreground">Client</p>
                    <p class="font-medium">
                        {{ verserTarget.client?.nom_complet }}
                    </p>
                    <div class="mt-2 flex gap-6 text-xs">
                        <div>
                            <p class="text-muted-foreground">Total</p>
                            <p class="font-semibold tabular-nums">
                                {{ formatGNF(verserTarget.montant) }}
                            </p>
                        </div>
                        <div>
                            <p class="text-muted-foreground">Déjà versé</p>
                            <p class="font-semibold tabular-nums">
                                {{ formatGNF(verserTarget.montant_verse) }}
                            </p>
                        </div>
                        <div>
                            <p class="text-muted-foreground">Restant</p>
                            <p
                                class="font-semibold text-orange-600 tabular-nums"
                            >
                                {{ formatGNF(verserTarget.montant_restant) }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    />
                    <p
                        v-if="verserForm.errors.mode_paiement"
                        class="mt-1 text-xs text-red-500"
                    >
                        {{ verserForm.errors.mode_paiement }}
                    </p>
                </div>
                ...
                <div>
                    ...
                    />
                    <p
                        v-if="verserForm.errors.date_versement"
                        class="mt-1 text-xs text-red-500"
                    >
                        {{ verserForm.errors.date_versement }}
                    </p>
                </div>
            ...
            <template #footer>
                <div class="flex justify-end gap-2">
                    <Button variant="outline" @click="verserVisible = false"
                        >Annuler</Button
                    >
                    <Button

...
<template>
    <div class="flex min-h-full flex-col bg-background">
        ...
        <div class="sticky top-0 z-20 border-b border-border/60 bg-background/95 backdrop-blur-sm">
            <div class="relative flex items-center justify-center px-4 pb-3 pt-4">
                <!-- Bouton retour -->
                <Link
                    href="/dashboard"
                ...
                <Link v-if="can('produits.create')" href="/produits/create" class="absolute right-4 shrink-0">
                    <button class="inline-flex items-center gap-1.5 rounded-full bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground shadow-sm transition-transform active:scale-95">
            ...
            >
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger as-child>
                        <button class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors active:bg-muted">
        ...
        <div v-else class="flex flex-col items-center gap-4 px-6 py-16 text-center">
            ...
            <Link v-if="can('produits.create') && !search" href="/produits/create">
                <button class="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-sm transition-transform active:scale-95">
                    <Plus class="h-4 w-4" />

...
const actionMeta: Record<Action, { label: string; color: string }> = {
    peut_valider: { label: 'Peut valider', color: 'text-blue-600 dark:text-blue-400' },
};
                ...
                <div v-show="activeTab === 'types'" class="space-y-4">
                    <div class="flex items-start justify-between gap-4">
                        <p class="text-sm text-muted-foreground">
                            Classement des dépenses par concerné : véhicule, livreur, salarié, propriétaire ou interne.
                        </p>
                        ...
                        >
                            <template #header>
                                <div class="flex flex-wrap items-center gap-3">
                                    ...
                                    <Select v-model="selectedCategorie" :options="categorieOptions" option-label="label" option-value="value" class="w-64" />
                                    <Select v-model="selectedStatut" :options="statutOptions" option-label="label" option-value="value" class="w-44" />
                            ...
                            <Column field="categorie_label" header="Concerné" sortable style="width: 140px">
                                <template #body="{ data }">
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium" :class="categorieColors[data.categorie] ?? 'bg-muted text-muted-foreground'">
                            ...
                            <Column header="" style="width: 110px">
                                <template #body="{ data }">
                                    <div class="flex justify-end gap-0.5">
                                        <button type="button" :title="data.is_active ? 'Désactiver' : 'Activer'" class="inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" @click="toggleType(data)">
                                            <Power class="h-3.5 w-3.5" />
                                        </button>
                                        <button type="button" title="Modifier" class="inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" @click="openEdit(data)">
                                            <Pencil class="h-3.5 w-3.5" />
                                        </button>
                                        <button type="button" title="Supprimer" class="inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive" @click="destroyType(data)">
                    ...
                    <div class="overflow-hidden rounded-xl border bg-card shadow-sm">
                            ...
                            <table class="w-full">
                                <thead>
                                    <tr class="border-b bg-muted/20">
                                        ...
                                        <th v-for="action in ACTIONS" :key="action" class="px-8 py-4 text-center" style="width: 160px">
                                            <div class="flex flex-col items-center gap-2">
                                                <span class="text-xs font-semibold tracking-wider uppercase" :class="actionMeta[action].color">
                                                ...
                                                >
                                                    <Check v-if="columnState(action) === 'all'" class="h-4 w-4" />
                                                    <Minus v-else-if="columnState(action) === 'partial'" class="h-4 w-4 text-primary" />
                                                </button>
                                            </div>
                                        </th>
                                        <!-- Colonne portée validation -->
                                        <th class="px-6 py-4 text-left" style="min-width: 220px">
                                            <span class="text-xs font-semibold tracking-wider text-muted-foreground uppercase">De quelles agences peut-il valider ?</span>
                                        </th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    <template v-for="entry in droitsForm" :key="entry.role_name">
                                        ...
                                        <tr
                                            ...
                                            :class="isAdminRole(entry.role_name) ? 'bg-muted/10' : 'hover:bg-muted/20'"
                                        >
                                            <!-- Rôle -->
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    <div
                                                        ...
                                                        :class="isAdminRole(entry.role_name) ? 'bg-blue-50 dark:bg-blue-950/40' : 'bg-muted/40'"
                                                    >
                                                    ...
                                                    <div>
                                                        ...
                                                        <p v-if="isAdminRole(entry.role_name)" class="text-xs text-muted-foreground">
                                            ...
                                            <td class="px-8 py-4 text-center">
                                                <div class="flex justify-center">
                                                    <div
                                                        v-if="isAdminRole(entry.role_name)"
                                                    ...
                                                    <button
                                                        ...
                                                        :class="
                                                            entry.peut_valider
                                                                ...
                                                                : 'border-border bg-background hover:border-primary/60'
                                                        "
                                                        @click="toggle(entry, 'peut_valider')"
                                                    >
                                            ...
                                            <!-- Portée -->
                                            <td class="px-6 py-4">
                                                <div v-if="isAdminRole(entry.role_name)" class="text-xs text-muted-foreground italic">
                                                    Toutes les agences
                                                </div>
                                                <button
                                                    ...
                                                    @click="togglePortee(entry.role_name)"
                                                >
                                        ...
                                        <tr
                                            v-if="!isAdminRole(entry.role_name) && entry.peut_valider && isPorteeExpanded(entry.role_name)"
                                            ...
                                            <td colspan="3" class="px-8 py-4">
                                                <div class="space-y-3">
                                                    ...
                                                    <div class="space-y-1.5">
                                                        <p class="text-xs font-medium text-muted-foreground">De quelles agences peut-il valider ?</p>
                                                        <div class="flex flex-wrap gap-2">
                                                            <button
                                                                type="button"
                                                                class="rounded-full border px-3 py-1 text-xs font-medium transition-colors"
                                                                :class="
                                                                    entry.perimetre === 'toutes_agences'
                                                                        ? 'border-primary bg-primary text-primary-foreground'
                                                                        : 'border-border text-muted-foreground hover:border-primary/40 hover:text-foreground'
                                                                "
                                                                @click="setPerimetre(entry, 'toutes_agences')"
                                                            >
                                                                Toutes les agences
                                                            </button>
                                                            <button
                                                                ...
                                                                class="rounded-full border px-3 py-1 text-xs font-medium transition-colors"
                                                                :class="
                                                                    entry.perimetre === 'son_agence'
                                                                        ? 'border-primary bg-primary text-primary-foreground'
                                                                        : 'border-border text-muted-foreground hover:border-primary/40 hover:text-foreground'
                                                                "
                                                                @click="setPerimetre(entry, 'son_agence')"
                                                            >
                                                            ...
                                                            <button
                                                                ...
                                                                class="rounded-full border px-3 py-1 text-xs font-medium transition-colors"
                                                                :class="
                                                                    entry.perimetre === 'agences_selectionnees'
                                                                        ? 'border-primary bg-primary text-primary-foreground'
                                                                        : 'border-border text-muted-foreground hover:border-primary/40 hover:text-foreground'
                                                                "
                                                                @click="setPerimetre(entry, 'agences_selectionnees')"
                                                            >
                                                    ...
                                                    <!-- Grid agences -->
                                                    <div
                                                        v-if="entry.perimetre === 'agences_selectionnees'"
                                                        class="rounded-lg border border-dashed bg-muted/20 px-4 py-3"
                                                    >
                                                        <div v-if="sites.length === 0" class="text-xs text-muted-foreground italic">
                                                        ...
                                                        <div v-else class="grid grid-cols-2 gap-1 sm:grid-cols-3 lg:grid-cols-4">
                                                            <button
                                                                ...
                                                                :class="hasSite(entry, site.id) ? 'text-primary' : 'text-muted-foreground'"
                                                                @click="toggleSite(entry, site.id)"
                                                            >
                                                                <div
                                                                    class="flex h-4 w-4 shrink-0 items-center justify-center rounded border-2 transition-all"
                                                                    :class="hasSite(entry, site.id) ? 'border-primary bg-primary text-primary-foreground' : 'border-border'"
                                                                >
                        ...
                        <div class="flex items-center gap-4 border-t bg-muted/20 px-6 py-3 text-xs text-muted-foreground">
                            <span class="flex items-center gap-1.5">
                                <span class="inline-block h-2 w-2 rounded-sm bg-primary" />
                                Accordé
                            </span>
                            <span class="flex items-center gap-1.5">
                                <span class="inline-block h-2 w-2 rounded-sm border border-border bg-background" />
                                Non accordé
                            </span>
                            <span class="flex items-center gap-1.5">
    ...
    >
        <form class="space-y-4 pt-2 pb-1" @submit.prevent="handleSubmit">
            <div>
                ...
                />
                <p v-if="typeForm.errors.libelle" class="mt-1 text-xs text-destructive">{{ typeForm.errors.libelle }}</p>
            </div>
            ...
            <div>
                ...
                <p v-if="typeForm.errors.categorie" class="mt-1 text-xs text-destructive">{{ typeForm.errors.categorie }}</p>
            </div>
            ...
            <div class="space-y-2.5 rounded-lg border bg-muted/30 p-3">
                ...
                <div class="flex items-center gap-3">
                    <Checkbox id="dt-comment" :model-value="typeForm.commentaire_obligatoire" @update:model-value="typeForm.commentaire_obligatoire = $event === true" />
                    <div>
                ...
                <div class="flex items-center gap-3">
                    <Checkbox id="dt-justif" :model-value="typeForm.justificatif_obligatoire" @update:model-value="typeForm.justificatif_obligatoire = $event === true" />
                    <div>
                ...
                <div class="flex items-center gap-3">
                    <Checkbox id="dt-active" :model-value="typeForm.is_active" @update:model-value="typeForm.is_active = $event === true" />
                    <div>
            ...
            <div class="flex justify-between pt-2">
                ...
                <Button type="submit" size="sm" :disabled="typeForm.processing">
                    {{ typeForm.processing ? 'Enregistrement…' : editingType ? 'Enregistrer' : 'Créer' }}
                </Button>
            </div>
        </form>
    </Dialog>

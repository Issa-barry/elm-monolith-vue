...
<template>
    <form id="proprietaire-form" class="space-y-4 sm:space-y-6" @submit.prevent="emit('submit')">
        <!-- Identité -->
        <div class="rounded-xl border bg-card p-4 shadow-sm sm:p-6">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <InputText id="prenom" :model-value="form.prenom" @update:model-value="
                        $emit('update:form', {
                            ...form,
                            prenom: String($event ?? ''),
                        })
                        " class="w-full" :class="{ 'p-invalid': errors.prenom }" />
                ...
                <div>
                    ...
                    <InputText id="nom" :model-value="form.nom" @update:model-value="
                        $emit('update:form', {
                            ...form,
                            nom: String($event ?? ''),
                        })
                        " class="w-full" :class="{ 'p-invalid': errors.nom }" />
        ...
        <div class="rounded-xl border bg-card p-4 shadow-sm sm:p-6">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <Dropdown :model-value="form.pays" @update:model-value="onPaysChange($event)"
                        :options="PAYS_OPTIONS" option-label="label" option-value="value" placeholder="Sélectionner..."
                        class="w-full" :class="{ 'p-invalid': errors.code_pays }">
                    ...
                    <p v-if="errors.code_pays" class="mt-1 text-xs text-destructive">
                        {{ errors.code_pays }}
                    </p>
                </div>
                <div>
                    <Label for="ville" class="mb-1.5 block">Ville</Label>
                    <InputText id="ville" :model-value="form.ville ?? ''" @update:model-value="
                        $emit('update:form', {
                            ...form,
                            ville: $event || null,
                        })
                        " class="w-full" :class="{ 'p-invalid': errors.ville }" />
                    <p v-if="errors.ville" class="mt-1 text-xs text-destructive">
                        {{ errors.ville }}
                    </p>
                </div>
                <div class="sm:col-span-2">
                    <Label for="adresse" class="mb-1.5 block">Adresse</Label>
                    <InputText id="adresse" :model-value="form.adresse ?? ''" @update:model-value="
                        $emit('update:form', {
                            ...form,
                            adresse: $event || null,
                        })
        ...
        <div class="rounded-xl border bg-card p-4 shadow-sm sm:p-6">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <div class="flex gap-2">
                        <div
                            ...
                            <img v-if="selectedCountry" :src="flagUrl(selectedCountry.code)"
                                :alt="selectedCountry.label" class="h-4 w-auto rounded-sm shadow-sm" />
                            <span>{{ form.code_phone_pays ?? '—' }}</span>
                        </div>
                        <InputText id="telephone" :model-value="form.telephone ?? ''"
                            @update:model-value="onTelephoneInput($event)" @keydown="handlePhoneKeydown"
                            :placeholder="`${selectedPhoneLength} chiffres`" inputmode="numeric" pattern="[0-9]*"
                            autocomplete="tel-national" :maxlength="phoneMaxLength" class="w-full"
                ...
                <div>
                    ...
                    <InputText id="email" :model-value="form.email ?? ''" @update:model-value="
                        $emit('update:form', {
                            ...form,
                            email: $event || null,
                        })
                        " type="email" class="w-full" :class="{ 'p-invalid': errors.email }" />
        ...
        <div class="rounded-xl border bg-card p-4 shadow-sm sm:p-6">
            ...
            <div class="flex items-center gap-3">
                <Checkbox id="is_active" :model-value="Boolean(form.is_active)" @update:model-value="
                    $emit('update:form', {
                        ...form,

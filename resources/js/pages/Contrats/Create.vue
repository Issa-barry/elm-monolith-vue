    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            <div class="mb-6 flex items-center gap-4">
                ...
                <h1 class="text-2xl font-semibold tracking-tight">Nouveau contrat</h1>
            </div>
            ...
            <form class="max-w-xl space-y-6" @submit.prevent="submit">
                <div class="rounded-xl border bg-card p-6 shadow-sm space-y-5">
                    ...
                    <div>
                        ...
                        <Select v-model="form.employe_id" :options="employes" option-label="label" option-value="value" filter placeholder="Sélectionner un employé" class="w-full" />
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <p v-if="form.errors.type_contrat" class="mt-1 text-xs text-destructive">{{ form.errors.type_contrat }}</p>
                        </div>
                        <div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input v-model="form.salaire_base" type="number" min="0" step="1000" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" :class="{ 'border-destructive': form.errors.salaire_base }" />
                            <p v-if="form.errors.salaire_base" class="mt-1 text-xs text-destructive">{{ form.errors.salaire_base }}</p>
                        </div>
                    </div>
                </div>

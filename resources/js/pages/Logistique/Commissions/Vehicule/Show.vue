...
import { ArrowLeft, HandCoins, History, Truck, User } from 'lucide-vue-next';
...
const grandPending = computed(
    () => livreurTotals.value.pending + propriTotals.value.pending,
);
const grandAvailable = computed(
    () => livreurTotals.value.available + propriTotals.value.available,
);
const grandPaid = computed(
    () => livreurTotals.value.paid + propriTotals.value.paid,
);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-0.5 text-xl font-semibold">
                            {{ vehicule.nom }}
                            <span
                                v-if="vehicule.immatriculation"
                                class="ml-2 font-mono text-sm font-normal text-muted-foreground"
                            >
                    ...
                    <table v-if="livreurs.length > 0" class="w-full text-sm">
                            ...
                            >
                                ...
                                <td class="px-4 py-3 text-center">
                                    ...
                                    >
                                        ...
                                        <Button
                                            v-if="
                                                can_payer && row.available > 0
                                            "
                                            size="sm"
                                            class="h-7 px-2.5"
                                            @click="openPaiement(row)"
                                        >
                                            Payer
                                        </Button>
                                        <Button
                        ...
                        <tfoot>
                            <tr class="border-t-2 bg-muted/20 font-semibold">
                                ...
                                >
                                    {{ formatGNF(livreurTotals.pending) }}
                                </td>
                                <td
                                    class="px-4 py-2.5 text-right tabular-nums"
                                    :class="
                                        livreurTotals.available > 0
                                            ? 'text-amber-600 dark:text-amber-400'
                                            : 'text-muted-foreground'
                                    "
                                >
                        ...
                        <tfoot>
                            <tr class="border-t-2 bg-muted/20 font-semibold">
                                ...
                                <td
                                    class="px-4 py-2.5 text-right text-muted-foreground tabular-nums"
                                >
                                    {{ formatGNF(propriTotals.pending) }}
                                </td>
                                <td
                                    class="px-4 py-2.5 text-right tabular-nums"
                                    :class="
                                        propriTotals.available > 0
                                            ? 'text-amber-600 dark:text-amber-400'
                                            : 'text-muted-foreground'
                                    "
                                >
        ...
        >
            <div class="space-y-4 py-2">
                ...
                >
                    Solde disponible :
                    <strong>{{ formatGNF(dialogRow?.available ?? 0) }}</strong>
                </div>
                <div>
        ...
        <Dialog
            ...
            :header="
                historiqueFiltre
                    ? `Historique — ${historiqueFiltre.nom}`
                    : `Tous les paiements — ${vehicule.nom}`
            "

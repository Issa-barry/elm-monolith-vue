...
const concerneReel = computed<string | null>(() => {
    ...
    if (props.vehiculeContext === 'externe_avec_proprietaire')
        return props.vehicule?.proprietaire_nom ?? null;
    if (props.vehiculeContext === 'externe_sans_proprietaire') return null;
    if (props.categorie === 'interne')
        return props.siteNom ? `Agence — ${props.siteNom}` : 'Agence ELM';
...
const montantFormate = computed<string | null>(() => {
    const n =
        typeof props.montant === 'number'
            ? props.montant
            : parseFloat(String(props.montant));
    ...
    <div class="rounded-xl border bg-card shadow-sm">
        ...
        <div v-if="!hasContent" class="px-4 py-10 text-center">
            ...
            <p class="text-sm text-muted-foreground">
                Sélectionnez une catégorie et un type de dépense pour afficher
                le récapitulatif.
            </p>
        </div>
        ...
        <div v-else class="divide-y divide-border">
            ...
            <div class="space-y-2 px-4 py-3">
                ...
                <div class="space-y-1 text-sm">
                    <div
                        v-if="categorie === 'vehicule' && vehicule"
            ...
            <div class="space-y-2 px-4 py-3">
                ...
                <div class="space-y-1 text-sm">
                    ...
                    <div v-if="commentaire" class="pt-1">
                        ...
                        >
                        <p
                            class="mt-0.5 line-clamp-3 text-xs leading-relaxed text-foreground/80"
                        >
                            {{ commentaire }}
                        </p>
                    </div>
            ...
            <div v-if="impactConfig" class="px-4 py-3">
                ...
                <div
                    ...
                    :class="impactConfig.cls"
                >
                    <CheckCircle
                        v-if="impactConfig.icon === 'check'"
                        class="mt-0.5 h-3.5 w-3.5 shrink-0"
                    />
                    <AlertCircle
                        v-else-if="impactConfig.icon === 'alert'"
                        class="mt-0.5 h-3.5 w-3.5 shrink-0"
                    />

...
onMounted(() => {
    scrollerRef.value?.addEventListener('scroll', updateCurrentSlide, { passive: true });
    updateCurrentSlide();
});
onUnmounted(() => {
...
<template>
        ...
        >
            ...
            <div data-slide class="min-w-[88%] snap-start">
                <div class="card relative h-full overflow-hidden border-transparent">
                    <svg viewBox="0 0 900 600" xmlns="http://www.w3.org/2000/svg"
                        class="absolute top-0 left-0 h-full w-full" preserveAspectRatio="none">
                        <rect x="0" y="0" width="900" height="600" fill="var(--p-primary-600)" />
                    ...
                    <div class="relative z-20 text-white">
                        ...
                        <div class="mb-4 text-2xl font-bold">{{ formatGNF(stats.total_montant) }}</div>
                        <div class="text-sm">{{ stats.total_count }} facture{{ stats.total_count !== 1 ? 's' : '' }}</div>
                    </div>
                </div>
            </div>
            ...
            <div data-slide class="min-w-[88%] snap-start">
                <div class="card flex h-full flex-col justify-center">
                    <span class="mb-3 text-xl font-medium text-foreground">Factures payées</span>
                    <span class="mb-3 text-2xl font-bold text-primary">{{ formatGNF(stats.payees_montant) }}</span>
                    <span class="text-sm text-muted-foreground">
                        {{ stats.payees_count }} facture{{ stats.payees_count !== 1 ? 's' : '' }} payée{{ stats.payees_count !== 1 ? 's' : '' }}
                    </span>
                </div>
            </div>
            ...
            <div data-slide class="min-w-[88%] snap-start">
                <div class="card flex h-full flex-col justify-center">
                    <span class="mb-3 text-xl font-medium text-foreground">Reste à encaisser</span>
                    <span class="mb-3 text-2xl font-bold text-primary">{{ formatGNF(stats.reste_a_encaisser) }}</span>
                    <span class="text-sm text-muted-foreground">
                        {{ stats.impayees_count }} impayée{{ stats.impayees_count !== 1 ? 's' : '' }}
                        — {{ stats.annulees_count }} annulée{{ stats.annulees_count !== 1 ? 's' : '' }}
                    </span>
                </div>
            </div>
        </div>
    ...
    <div class="col-span-12 hidden sm:block md:col-span-6 xl:col-span-4">
        <div class="card relative h-full overflow-hidden border-transparent">
            ...
            <div class="relative z-20 text-white">
                <div class="mb-4 text-xl font-semibold">Total Factures</div>
                <div class="mb-6 text-2xl font-bold">{{ formatGNF(stats.total_montant) }}</div>
                <div class="text-sm">{{ stats.total_count }} facture{{ stats.total_count !== 1 ? 's' : '' }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="col-span-12 hidden sm:block md:col-span-6 xl:col-span-4">
        <div class="card flex h-full flex-col items-center justify-center text-center">
            <span class="mb-4 text-xl font-medium text-foreground">Factures payées</span>
            <span class="mb-4 text-2xl font-bold text-primary">{{ formatGNF(stats.payees_montant) }}</span>
            <span class="text-sm text-muted-foreground">
                {{ stats.payees_count }} facture{{ stats.payees_count !== 1 ? 's' : '' }} payée{{ stats.payees_count !== 1 ? 's' : '' }}
            </span>
        </div>
    ...
    <div class="col-span-12 hidden sm:block md:col-span-6 xl:col-span-4">
        <div class="card flex h-full flex-col items-center justify-center text-center">
            <span class="mb-4 text-xl font-medium text-foreground">Reste à encaisser</span>
            <span class="mb-4 text-2xl font-bold text-primary">{{ formatGNF(stats.reste_a_encaisser) }}</span>
            <span class="text-sm text-muted-foreground">
                {{ stats.impayees_count }} impayée{{ stats.impayees_count !== 1 ? 's' : '' }}
                — {{ stats.annulees_count }} annulée{{ stats.annulees_count !== 1 ? 's' : '' }}
            </span>
